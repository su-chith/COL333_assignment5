...
double evaluate(Board b, PlayerColor clr) {
    ...
    if (b.in_check()) {
        if (b.data.player_to_play == WHITE) white_score -= 2.0;
...
double evaluate_2(Board b, PlayerColor clr, U8 move) {
    ...
    if (piece_type & PAWN) {
        ...
        if (threat) {
            if (piece_type & WHITE) white_score -= 10.0;
            else black_score -= 10.0;
        }   
        if (threat_0) {
            if (piece_type & WHITE) white_score += 10.0;
            else black_score += 10.0;
        ...
        if (threat) {
            if (piece_type & WHITE) white_score -= 15.0;
            else black_score -= 15.0;
        }
        if (threat_0) {
            if (piece_type & WHITE) white_score += 15.0;
            else black_score += 15.0;
